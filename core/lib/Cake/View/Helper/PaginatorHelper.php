...
//<--
// Siva // Commented due to page URL appended with sorting URL
	...
	public function sort($key, $title = null, $options = array()) {
		...
		if ($isSorted) {
			...
			if (!empty($options['class'])) {
				$options['class'] .= 'js-sorting ' . $class;
			} else {
				$options['class'] = 'js-sorting '. $class;
			}
		}
 ...
 */
	protected function _pagingLink($which, $title = null, $options = array(), $disabledTitle = null, $disabledOptions = array()) {
		...
		$span_class = $class;
		if ($this->{$check}($model)) {
			return $this->Html->tag($tag, '<span class="'.$span_class.'">'.$this->link($title, $url, array_merge($options, compact('escape', 'class'))).'</span>');
		} else {
			unset($options['rel']);
			$class = 'disabled';		
			return $this->Html->tag($tag, '<span class="'.$span_class.'">'.$title.'</span>', array_merge($options, compact('escape', 'class')));
		}
	}
 ...
 */
	public function numbers($options = array()) {
		...
		unset($options['tag'], $options['before'], $options['after'], $options['model'],
			...
			$options['ellipsis'], $options['currentClass']
		);
		...
		if ($modulus && $params['pageCount'] > $modulus) {
			...
			$out .= $this->Html->tag($tag, '<span>'.$params['page'].'</span>', array('class' => $currentClass));
 ...
 */
	public function first($first = '<< first', $options = array()) {
		...
		if (is_int($first) && $params['page'] >= $first) {
			...
			$out = $this->Html->tag($tag, $this->link($first, array('page' => 1), $options), compact('class')) . "<li>".$after.'</li>';
		}
		return $out;
	}
 ...
 */
	public function last($last = 'last >>', $options = array()) {
		...
		if (is_int($last) && $params['page'] <= $lower) {
			...
			$out = "<li>".$before.'</li>' . $out;
		} elseif ($params['page'] < $params['pageCount'] && is_string($last)) {
			$options += array('rel' => 'last');
			$out = "<li>".$before.'</li>' . $this->Html->tag(
