...
class GoogleAnalyticsController extends AppController
{
    ...
    public function admin_chart_bounces() 
    {
        ...
        foreach($this->$select_var as $display => $condition) {
            $stats = $this->_getStats($condition, 'ga:pageviews,ga:visitors,ga:bounces,ga:entrances', array(
                'dimensions' => 'ga:date',
                'sort' => 'ga:date'
            ));
        ...
        if (!empty($prev_total_pageviews) && !empty($total_pageviews)) {
            $pageviews_percentage = round((($total_pageviews-$prev_total_pageviews) *100) /$prev_total_pageviews);
        } else if (empty($prev_total_pageviews) && !empty($total_pageviews)) {
            $pageviews_percentage = 100;
        } else {
            $pageviews_percentage = 0;
        }
        $this->set('pageviews_percentage', $pageviews_percentage);
        if (!empty($prev_total_visitors) && !empty($total_visitors)) {
            $visitors_percentage = round((($total_visitors-$prev_total_visitors) *100) /$prev_total_visitors);
        } else if (empty($prev_total_visitors) && !empty($total_visitors)) {
            $visitors_percentage = 100;
        } else {
            $visitors_percentage = 0;
        }
        $this->set('visitors_percentage', $visitors_percentage);
        ...
        if (!empty($prev_total_bounce_rate) && !empty($total_bounce_rate)) {
            $bounce_rate_percentage = round((($total_bounce_rate-$prev_total_bounce_rate) *100) /$prev_total_bounce_rate);
        } else if (empty($prev_total_bounce_rate) && !empty($total_bounce_rate)) {
            $bounce_rate_percentage = 100;
        } else {
            $bounce_rate_percentage = 0;
        }
        $this->set('bounce_rate_percentage', $bounce_rate_percentage);
    ...
    public function admin_chart_visitors() 
    {
        ...
        foreach($this->$select_var as $display => $condition) {
            $stats = $this->_getStats($condition, 'ga:visits,ga:newVisits', array(
                'dimensions' => 'ga:date',
                'sort' => 'ga:date'
            ));
    ...
    {
        $stats = $this->analytics->data_ga->get('ga:' . Configure::read('google_analytics.profile_id') , $condition['start_date'], $condition['end_date'], $metric, $params);
        return $stats;
    }
    ...
    public function admin_form_bounce_chart() 
    {
        ...
        $this->loadModel('Projects.ProjectType');
        ...
        foreach($projectTypes as $type) {
            ...
            $project_add_form_bounce_arr = array();
            $project_fund_form_bounce_arr = array();
            // project add form bounce query
            $stats = $this->_getStats($condition, 'ga:totalEvents', array(
                'dimensions' => 'ga:eventLabel',
                'sort' => 'ga:eventLabel',
                'filters' => 'ga:eventCategory=~Project;ga:eventAction=~' . $type_name . 'Posted;ga:eventLabel=~Step '
            ));
            $project_add_steps_arr = array();
            $project_add_events_arr = array();
            if (!empty($stats->rows)) {
                foreach($stats->rows as $stat) {
                    $project_add_steps_arr[] = "'" . $stat[0] . "'";
                    $project_add_events_arr[] = $stat[1];
                }
            }
            $project_add_form_bounce_arr[] = '["Form", ' . implode(', ', $project_add_steps_arr) . ']';
            $project_add_form_bounce_arr[] = '["' . $type_name . ' Project Post", ' . implode(', ', $project_add_events_arr) . ']';
            $this->set($type_name . 'project_add_form_bounces', implode(',', $project_add_form_bounce_arr));
        }
        // project fund form bounce query
        $stats = $this->_getStats($condition, 'ga:totalEvents', array(
            'dimensions' => 'ga:eventLabel',
            'sort' => 'ga:eventLabel',
            'filters' => 'ga:eventCategory=~ProjectFund;ga:eventAction=~Fund;ga:eventLabel=~Step '
        ));
        $project_fund_events_arr = array();
        $project_fund_steps_arr = array();
        if (!empty($stats->rows)) {
            foreach($stats->rows as $stat) {
                $project_fund_steps_arr[] = "'" . $stat[0] . "'";
                $project_fund_events_arr[] = $stat[1];
            }
            if (!empty($project_fund_events_arr)) {
                $project_fund_form_bounce_arr[] = '["Form", ' . implode(', ', $project_fund_steps_arr) . ']';
                $project_fund_form_bounce_arr[] = '["Project Fund", ' . implode(', ', $project_fund_events_arr) . ']';
            }
        }
        $this->set($type_name . 'project_fund_form_bounces', implode(',', $project_fund_form_bounce_arr));
    }
    public function admin_sources_chart() 
    {
        ...
        $visit_by_sources_stats = $this->_getStats($condition, 'ga:visits', array(
            'dimensions' => 'ga:source',
            'sort' => 'ga:source'
        ));
        $this->set('visit_by_sources_stats', $visit_by_sources_stats);
        // country query
        $countries_stats = $this->_getStats($condition, 'ga:visitors', array(
    ...
    public function admin_ecommerce_chart() 
    {
        ...
        // ecommerce query
        $ecommerce_stats = $this->_getStats($condition, 'ga:totalValue', array(
            'dimensions' => 'ga:source'
        ));
        $ecommerce_stats_category = $this->_getStats($condition, 'ga:itemRevenue', array(
    ...
    public function admin_ecommerce_transaction_chart() 
    {
        ...
        foreach($this->$select_var as $display => $condition) {
            $stats = $this->_getStats($condition, 'ga:transactions,ga:transactionRevenue', array(
                'dimensions' => 'ga:date',
                'sort' => 'ga:date'
            ));
        ...
        if (!empty($prev_total_transaction) && !empty($total_transaction)) {
            $transaction_percentage = round((($total_transaction-$prev_total_transaction) *100) /$prev_total_transaction);
        } else if (empty($prev_total_transaction) && !empty($total_transaction)) {
            $transaction_percentage = 100;
        } else {
            $transaction_percentage = 0;
        }
        $this->set('transaction_percentage', $transaction_percentage);
        if (!empty($prev_total_transactionRevenue) && !empty($total_transactionRevenue)) {
            $transactionRevenue_percentage = round((($total_transactionRevenue-$prev_total_transactionRevenue) *100) /$prev_total_transactionRevenue);
        } else if (empty($prev_total_transactionRevenue) && !empty($total_transactionRevenue)) {
            $transactionRevenue_percentage = 100;
        } else {
            $transactionRevenue_percentage = 0;
        }
        $this->set('transactionRevenue_percentage', $transactionRevenue_percentage);
