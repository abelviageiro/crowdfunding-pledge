...
class Subscription extends AppModel
{
	...
	public function SubscribeMailonLaunch($launch_type) 
    {
        App::import('Model', 'EmailTemplate');
        $this->EmailTemplate = new EmailTemplate();
        if ($launch_type == 'Pre-launch' || $launch_type == 'Private Beta') {
            $email_template = $this->EmailTemplate->selectTemplate('Launch mail');
        }
        if ($launch_type == 'private_beta') {
        ...
        foreach($subscriptions as $subscription) {
            ...
            $this->_sendEmail($email_template, $emailFindReplace, $subscription['Subscription']['email']);
            if ($launch_type == 'private_beta') {
