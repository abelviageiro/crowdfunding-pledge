...
<div class="userTypes index js-response">
	<table class="list">
		<tr>
			...
			{ ?>
			<th class="role-name"><?php echo $this->Html->cText($role['Role']['name'], false); ?></th>
			<?php }?>
		</tr>
		<?php
		if (!empty($aclLinks)):
			...
			<?php  
				...
				{ 
					...
					{?>
					<tr class="hide <?php echo $this->Html->cText($aclLink['AclLink']['controller'], false); ?>"> 
					<td class="acl-name"><?php echo $this->Html->cText($aclLink['AclLink']['name']); ?></td>
					<?php 
