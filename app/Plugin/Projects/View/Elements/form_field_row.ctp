<?php
  ...
  if (!empty($field['is_deletable'])) {
    $row_delete = $this->Html->link('<i class="fa fa-times"></i> ' . __l('Remove'), array('controller' => 'form_fields', 'action' => 'delete', $field['id']), array('class' => 'js-no-pjax js-form-field-delete', 'title' => __l('Remove'), 'escape' => false));
  } else {
    $row_delete = '<span class="label label-info js-tooltip" title="' . __l('Core, cannot be deleted') . '">' . __l('Core') . '</span>';
  }
  $row_active = '-';
  if (!empty($field['is_deletable'])) {
    $row_active = $this->Form->input('FormField.' . $key . '.is_active', array('label' => false, 'checked' => ($field['is_active'] ? 'checked' : '')));
  }
  $row_display_text ='-';
  ...
  $row = array(
    ...
    $this->Form->hidden('FormField.' . $key . '.project_type_id', array('value' => $field['project_type_id'])) .
	...
	$this->Form->input('FormField.' . $key . '.is_editable', array('label' => false, 'checked' => ($field['is_editable']?'checked':''))),
    $row_active,
    $row_delete . $this->Html->link(' <i class="fa fa-pencil-square-o fa-fw"></i> ' . __l('Depends On'), array('controller' => 'form_fields', 'action' => 'edit', $field['id']), array('data-toggle' => 'modal', 'data-target' => '#js-edit-ajax-modal', 'class' => 'js-no-pjax' ,'title' => __l('Depends On'), 'escape' => false))
  );
