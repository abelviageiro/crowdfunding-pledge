...
class FormFieldsController extends AppController
{
    ...
    public function admin_add($formId = null)
    {
        ...
        if (!empty($this->request->data)) {
            ...
            if ($this->FormField->save($this->request->data)) {
                if ($this->RequestHandler->isAjax()) {
                    ...
                    $this->redirect(array(
                        ...
                        'action' => 'edit',
                        $this->request->data['FormField']['project_type_id'],
                        'type' => 'form_fields'
                    ));
                }
            } else {
            ...
            $this->request->data['FormField']['project_type_id'] = $formId;
            $types = $this->FormField->types;
            $this->set('types', $types);
        }
    ...
    public function admin_get_row($id)
    {
        ...
        $key = $this->FormField->find('count', array(
            'conditions' => array(
                'FormField.project_type_id' => $field['project_type_id']
            )
        ));
    ...
    public function admin_edit($id = null)
    {
        ...
        if (!$id && empty($this->request->data)) {
            ...
            if ($this->FormField->save($this->request->data, false)) {
                ...
                $this->Session->setFlash(sprintf(__l('%s has been updated') , __l('Form Field')) , 'default', null, 'success');
                $this->redirect(array(
                    'controller' => 'project_types',
                    'action' => 'edit',
                    $this->request->data['FormField']['project_type_id']
            ...
            $dependson_fields = $this->FormField->find('list', array(
                'conditions' => array(
                    'FormField.project_type_id' => $form_field['FormField']['project_type_id']
                ) ,
                'fields' => array(
                    'FormField.name',
                    'FormField.label'
