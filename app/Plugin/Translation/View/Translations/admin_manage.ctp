...
<div class="translations admin-form">
  ...
  <?php echo $this->Form->create('Translation', array('action' => 'manage', 'class' => 'form-horizontal')); ?>
	...
	<div class="gray-bg">
		  <?php
			echo $this->Form->input('language_id',array('label'=>__l('Language')));
			echo $this->Form->input('filter', array('type' => 'hidden'));
			echo $this->Form->input('q', array('label' => __l('Keyword')));
		  ?>
		  <div class="clearfix">
			<?php echo $this->Form->submit(__l('Submit'), array('name' => 'data[Translation][makeSubmit]','class'=>'btn btn-info')); ?>
		  </div>
	</div>
      <?php
      ...
      ?>
      <table class="table table-striped table-bordered table-condensed navbar-btn">
        ...
        ?>
        <tr>
          <td class="select"> <?php echo $this->Form->input('Translation.'.$translation['Translation']['id'].'.is_verified', array('checked' => ($translation['Translation']['is_verified'])?true:false, 'label' => '')); ?></td>
          <td><?php echo $this->Html->cText($translation['Translation']['name'], false); ?></td>
        ...
        ?>
        <tr>
          <td colspan="3"><?php echo $this->Form->submit(__l('Update'), array('name' => 'data[Translation][makeUpdate]')); ?></td>
        </tr>
        <?php
          else:
        ?>
        <tr>
          <td colspan="3" class="text-center text-danger text-12"><i class="fa fa-exclamation-triangle"></i> <?php echo sprintf(__l('No %s available'), __l('Translations'));?></td>
        </tr>
        <?php
          endif;
        ?>
