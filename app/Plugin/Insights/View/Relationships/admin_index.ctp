...
<div class="main-admn-usr-lst js-response">
	<div class="bg-primary row">		
		<ul class="list-inline sec-1 navbar-btn">
			...
			<li>
				<?php echo $this->Html->link('<span class="img-circle img-thumbnail bg-sucess img-wdt center-block text-center opn-i-usr">'.$this->Html->cInt($pending + $approved,false).'</span><span>' .__l('All'). '</span>', array('controller'=>'relationships','action'=>'index'), array('class' => 'text-center','escape' => false));?>
			</li>
		</ul>		
	</div>
	<div class="clearfix">		
			...
			<table class="table table-striped">
				<thead class="h5">
					<tr>      
						...
						<th class="text-left"><?php echo $this->Paginator->sort('relationship', __l('Relationship'));?> </th>
					</tr>
				</thead>
				<tbody class="h5">
					<?php
					...
					<tr <?php echo $disabled; ?>>
						...
						<?php echo $this->Form->input('Relationship.'.$userRelationship['Relationship']['id'].'.id', array('type' => 'checkbox', 'id' => "admin_checkbox_".$userRelationship['Relationship']['id'], 'label' => '', 'class' => $status_class.' js-checkbox-list')); ?>
						</td>
						<td class="text-center">
							<div class="dropdown">
								...
								<ul class="dropdown-menu">
									<li><?php echo $this->Html->link('<i class="fa fa-pencil-square-o fa-fw"></i>'.__l('Edit'), array( 'action'=>'edit', $userRelationship['Relationship']['id']), array('class' => '','escape'=>false, 'title' => __l('Edit')));?></li>
									<li><?php echo $this->Html->link('<i class="fa fa-times fa-fw"></i>'.__l('Delete'), Router::url(array('action'=>'delete',$userRelationship['Relationship']['id']),true).'?r='.$this->request->url, array('class' => 'js-confirm ', 'escape'=>false,'title' => __l('Delete')));?></li>
									<?php echo $this->Layout->adminRowActions($userRelationship['Relationship']['id']);  ?>
								</ul>
							</div>
						</td>
						<td class="text-left"><?php echo $this->Html->cText($userRelationship['Relationship']['relationship']);?></td>
					</tr>
					<?php
					endforeach;
					else:
					?>
					<tr>
					<td colspan="6" class="text-center text-danger"><i class="fa fa-exclamation-triangle fa-fw"></i> <?php echo sprintf(__l('No %s available'), __l('Relationships'));?></td>
					</tr>
					<?php
					endif;
					?>
