...
<div class="main-admn-usr-lst js-response">
	...
	<div class="clearfix">		
		...
		<?php
		echo $this->Form->create('Education' , array('action' => 'update', 'class' => 'js-shift-click js-no-pjax'));
			...
			<table class="table table-striped">
				<thead class="h5">
					<tr>      
						...
						<th class="text-left"><?php echo $this->Paginator->sort('education', __l('Education'));?></th>
					</tr>
				</thead>
				<tbody class="h5">
					<?php
					...
					if($userEducation['Education']['is_active']):
					...
					<tr <?php echo $disabled; ?>>
						<td  class="text-center">
							<?php echo $this->Form->input('Education.'.$userEducation['Education']['id'].'.id', array('type' => 'checkbox', 'id' => "admin_checkbox_".$userEducation['Education']['id'], 'label' => '', 'class' => $status_class.' js-checkbox-list')); ?>
						</td>
						<td class="text-center">
							<div class="dropdown">
								...
								<ul class="dropdown-menu">
									<li><?php echo $this->Html->link('<i class="fa fa-pencil-square-o fa-fw"></i>'.__l('Edit'), array( 'action'=>'edit', $userEducation['Education']['id']), array('class' => 'js-edit','escape'=>false, 'title' => __l('Edit')));?></li>
									<li><?php echo $this->Html->link('<i class="fa fa-times fa-fw"></i>'.__l('Delete'), Router::url(array('action'=>'delete',$userEducation['Education']['id']),true).'?r='.$this->request->url, array('class' => 'js-confirm ', 'escape'=>false,'title' => __l('Delete')));?></li>
									<?php echo $this->Layout->adminRowActions($userEducation['Education']['id']);  ?>
								</ul>
							</div>
						</td>
						<td class="text-left"><?php echo $this->Html->cText($userEducation['Education']['education']);?></td>
					</tr>
					<?php
					endforeach;
					else:
					?>
					<tr>
						<td colspan="6" class="text-center text-danger"><i class="fa fa-exclamation-triangle fa-fw"></i> <?php echo sprintf(__l('No %s available'), __l('Educations'));?></td>
					</tr>
					<?php
					endif;
					?>
