...
<div class="main-admn-usr-lst js-response">
	<div class="bg-primary row">		
		<ul class="list-inline sec-1 navbar-btn">
			...
			<li>
				<?php echo $this->Html->link('<span class="img-circle img-thumbnail bg-sucess img-wdt center-block text-center opn-i-usr">'.$this->Html->cInt($pending + $approved,false).'</span><span>' .__l('All'). '</span>', array('controller'=>'employments','action'=>'index'), array('class' => 'text-center','escape' => false));?>
			</li>
		</ul>		
	</div>
	<div class="clearfix">		
		...
		<?php
		echo $this->Form->create('Employment' , array('action' => 'update', 'class' => 'js-shift-click js-no-pjax'));
		...
		<div class="table-responsive">
			<table class="table table-striped">
				...
				<tbody class="h6">
					...
					if($userEmployment['Employment']['is_active']):
					...
					<tr <?php echo $disabled; ?>>
						<td  class="text-center"><?php echo $this->Form->input('Employment.'.$userEmployment['Employment']['id'].'.id', array('type' => 'checkbox', 'id' => "admin_checkbox_".$userEmployment['Employment']['id'], 'label' => '', 'class' => $status_class.' js-checkbox-list')); ?></td>
						<td class="text-center">
							<div class="dropdown">
								...
								<ul class="dropdown-menu">
									<li><?php echo $this->Html->link('<i class="fa fa-pencil-square-o fa-fw"></i>'.__l('Edit'), array( 'action'=>'edit', $userEmployment['Employment']['id']), array('class' => '','escape'=>false, 'title' => __l('Edit')));?></li>
									<li><?php echo $this->Html->link('<i class="fa fa-times fa-fw"></i>'.__l('Delete'), Router::url(array('action'=>'delete',$userEmployment['Employment']['id']),true).'?r='.$this->request->url, array('class' => 'js-confirm ', 'escape'=>false,'title' => __l('Delete')));?></li>
									<?php echo $this->Layout->adminRowActions($userEmployment['Employment']['id']);  ?>
								</ul>
							</div>
						</td>
						<td class="text-left"><?php echo $this->Html->cText($userEmployment['Employment']['employment']);?></td>
					</tr>
					<?php
					endforeach;
					else:
					?>
					<tr>
						<td colspan="6" class="text-center text-danger"><i class="fa fa-exclamation-triangle fa-fw"></i> <?php echo sprintf(__l('No %s available'), __l('Employment'));?></td>
					</tr>
					<?php
					endif;
					?>
