...
?>
   ...
   <p><i class="fa fa-exclamation-triangle"></i> <?php echo sprintf(__l('Sorry, admin declined your request. If you want submit once again please %s'), $this->Html->link(__l('click here'), array('controller' => 'affiliates', 'action' => 'index', 'status' =>'add'), array('title' => __l('click here')))); ?></p>

<?php
  elseif($status == 'add' || (!empty($this->request->params['named']['status']) &&  $this->request->params['named']['status'] == 'add')):
?>
<div class="disk-usage admin-form">

  <div class="alert alert-info"><?php echo __l('This request will be confirmed after admin approval.'); ?> </div>

    <?php echo $this->Form->create('AffiliateRequest', array('class' => 'form-horizontal gray-bg'));?>
      <?php
