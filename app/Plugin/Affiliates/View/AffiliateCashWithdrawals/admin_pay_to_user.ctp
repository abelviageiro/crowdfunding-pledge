...
<div class="affiliateCashWithdrawals index js-response">
  ...
  <table class="table table-striped table-bordered table-condensed table-hover">
    ...
    <tr>
      <td class="text-left">
        <div class="paypal-status-info">
          ...
          ?>
            <span class="paypal grid_right"><?php echo $this->Html->cText($moneyTransferAccount['PaymentGateway']['display_name']);?></span>
          <?php
              endif;
            endforeach;
          ?>
          ...
          <?php echo $this->Html->getUserAvatar($affiliateCashWithdrawal['User'], 'micro_thumb', true, '', 'admin');  ?>
          <?php echo $this->Html->getUserLink($affiliateCashWithdrawal['User']);?>
        </div>
      </td>
      <td class="text-left"><?php echo $this->Html->cCurrency($affiliateCashWithdrawal['AffiliateCashWithdrawal']['amount']);?></td>
      <td class="text-left">
      ...
      <td class="text-right"><?php echo $this->Html->siteCurrencyFormat($affiliateCashWithdrawal['User']['commission_paid_amount']); ?></td>
    </tr>
    <?php
        endforeach;
      else:
    ?>
    <tr>
      <td colspan="8"><i class="fa fa-exclamation-triangle"></i> <?php echo sprintf(__l('No %s available'), __l('Affiliate Cash Withdrawals'));?></td>
    </tr>
    <?php
      endif;
    ?>
