...
class Facebook
{
  ...
  public static $DOMAIN_MAP = array(
	...
	'graph_oauth'     => 'https://graph.facebook.com/oauth/',   
	'oauth'     => 'https://www.facebook.com/dialog/'
  );
  ...
  /**
   set access token
   */
   public function setAccessToken($params=array()){	  
	  ...
	  $result = $this->makeRequest($url, array());
	  if(stripos($result, 'access_token')!== false){
		  parse_str($result);
		  $graph_url = self::$DOMAIN_MAP['graph'].'me?access_token=' . $access_token;
          $user = json_decode(file_get_contents($graph_url));		  
		  $return = array();
		  $return['access_token'] = $access_token;
		  $return['expires'] = !empty($expires) ? $expires : 0;
		  ...
		  $return['id'] = $user->id;
		  $return['updated_time'] = $user->updated_time;
		  $_SESSION['fbuser'] = $return;
