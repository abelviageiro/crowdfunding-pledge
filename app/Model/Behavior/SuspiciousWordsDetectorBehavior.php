...
class SuspiciousWordsDetectorBehavior extends ModelBehavior
{
    ...
    function beforeSave(Model $Model) 
    {
        ...
        if (!empty($this->__settings[$Model->alias]['fields']) && $this->__badWords) {
            ...
            $suspend_array = array(
                'MessageContent' => Configure::read('Project.auto_suspend_message_on_system_flag') ,
                'Project' => Configure::read('Project.auto_suspend_project_on_system_flag') ,
                'BlogComment' => Configure::read('Project.auto_suspend_update_comment_on_system_flag') ,
                'Blog' => Configure::read('Project.auto_suspend_update_on_system_flag') ,
            );
            if (!empty($is_system_flagged)) {
                ...
                if ($suspend_array[$Model->alias]) { // During auto suspend turned on by admin
                    $Model->data[$Model->alias]['is_admin_suspended'] = 1;
                }
