        ...
        autogeocomplete: function(options){
            ...
            this.autocomplete({
                ...
                select: function(event, ui) {
					$('#address-info').hide();
					var new_address = ui.item.label + ' ' + ui.item.area;
					$('#address-info').removeClass('error-message');
                    $("#js-street_id").val(new_address);
					...
					var class_name = $("#flag-flag").attr('class');
					var country_id = ui.item.country;
					var new_class_name = 'flags flag-' + country_id.toLowerCase();
					$('#flag-flag').removeClass(class_name);
					$('#flag-flag').addClass(new_class_name);
			...
			.data( "ui-autocomplete" )._renderMenu = function( menuUl, items ) {
				  $.each( items, function( index, item ) {
					...
					var li = $( "<li></li>" )
					  ...
					  ul = $( "<ul></ul>" )
					  	.addClass('menu-over-ul dropdown-menu')
						.css("display","none")
						.appendTo(li);

					$( "<li></li>" )
					 .append( $( "<h5></h5>" ).addClass('space').text( 'Refine Address' ) )
					 .appendTo( ul );

					 $( "<li></li>" )
					 .addClass('menu-over-address')
					 .data( "ui-autocomplete-item", item )
					 .append( $( "<a class='js-no-pjax'></a>" ).text( generate_string).attr("tabindex","-1") )
					 ...
					 .addClass('menu-over-address')
					 .data( "ui-autocomplete-item", item )
					 .append( $( "<a class='js-no-pjax'></a>" ).text( item.value ) )
					 .click(function( event ) {
						  menuUl.menu( "activate", event, li );
						  li.data( "ui-autocomplete-item", {
