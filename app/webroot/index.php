...
function updateViews($main_table_name,$main_field_name,$view_table_name,$main_id)
{
	...
	if($database->default['datasource']=='Database/Mysql') {
		...
		if(stripos($view_table_name, 'project_views') !== false){
			...
			if ($main_field_name == 'user') {
				mysql_query('INSERT INTO `' . $view_table_name . '` (`created`, `modified`, `viewing_user_id`, `user_id`, `ip_id`) VALUES (now(), now(), ' . $user_id . ', ' . $main_id . ', ' . $ip_id . ')');
			} else {
		...
		if (stripos($view_table_name, 'project_views') !== false) {
			...
			if ($main_field_name == 'user') {
				pg_query($db, 'INSERT INTO ' . $view_table_name . ' ("created", "modified", "viewing_user_id", "user_id", "ip_id") VALUES (now(), now(), ' . $user_id . ', ' . $main_id . ', ' . $ip_id . ')');
			} else {
